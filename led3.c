...
void traverseList(TreeNode* root, uint32_t oddColors[], int num_odd_colors, uint32_t evenColors[], int num_even_colors) {
...
int main() {
    ...
    while (seconds_elapsed < 16) {
        if (seconds_elapsed % 2 == 0) {
            ...
            traverseList(root, oddColors, num_odd_colors, evenColors, num_even_colors);
        } else {
            // Odd seconds: Turn on odd lights
            traverseList(root, evenColors, num_even_colors, oddColors, num_odd_colors);
        }
